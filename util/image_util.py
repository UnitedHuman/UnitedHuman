...
def uncrop(img, center, scale, orig_shape, rot=0, is_rgb=True):
    ...
    #     new_shape += [img.shape[2]]
    if is_rgb:
        new_img = np.zeros([orig_shape[0], orig_shape[1], 3], dtype=np.uint8)
    else:
        new_img = np.zeros(orig_shape, dtype=np.uint8)
    # Range to fill new array
    new_x = [max(0, -ul[0]), min(min(br[0], orig_shape[1]) - ul[0], res[1])]
    new_y = [max(0, -ul[1]), min(min(br[1], orig_shape[0]) - ul[1], res[0])]
    # Range to sample from original image
    old_x = [max(0, ul[0]), min(orig_shape[1], br[0])]
    old_y = [max(0, ul[1]), min(orig_shape[0], br[1])]
    if old_y[1] - old_y[0] > res[0]:
        delta_y = old_y[1] - old_y[0] - res[0]
        old_y[1] -= delta_y
    if old_x[1] - old_x[0] > res[1]:
        delta_x = old_x[1] - old_x[0] - res[1]
        old_x[1] -= delta_x
    new_x = tuple(new_x)
    new_y = tuple(new_y)
    old_x = tuple(old_x)
    ...
    return new_img, [new_x, new_y, old_x, old_y]
